            ...
            <div className="mb-6">
              <div className="bg-white border border-border rounded-2xl p-5 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h2 className="font-sans text-lg font-semibold text-foreground">Favorites</h2>
                </div>
                <div className="flex flex-wrap gap-2">
                  {favoriteTemplates.slice(0, 5).map(template => (
                    <span
                      key={template.id}
                      className="px-3 py-1.5 bg-muted border border-border rounded-full text-sm font-medium text-foreground hover:bg-accent transition-colors cursor-pointer"
                    >
                      {template.name}
                    </span>
                  ))}
