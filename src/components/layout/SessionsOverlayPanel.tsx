...
import { X, Search, SlidersHorizontal, ArrowUpDown, RefreshCw, Plug, Trash2, CheckSquare } from 'lucide-react';
...
import { ScheduledMeetingCard } from '@/components/sessions/ScheduledMeetingCard';
...
export const SessionsOverlayPanel = () => {
  ...
  const handleSessionClick = (id: string) => {
    // Load session into workspace without closing overlay
    loadSessionIntoWorkspace(id);
  };
  ...
  return (
    <div className="h-full flex flex-col bg-white border-r border-border w-full relative">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                onClick={closeSessionsOverlay}
              >
                <X className="h-4 w-4" />
              </Button>
            </TooltipTrigger>
            <TooltipContent side="bottom" className="text-xs">Hide sessions</TooltipContent>
          </Tooltip>
        </TooltipProvider>
      </div>
      ...
      <Tabs 
        value={sessionsTab} 
        onValueChange={(v) => setSessionsTab(v as 'sessions' | 'scheduled' | 'drafts')} 
      ...
      >
        ...
        <TabsContent value="scheduled" className="flex-1 overflow-y-auto m-0 p-4 space-y-4">
